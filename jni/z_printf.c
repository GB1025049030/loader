...
static void putcharfd(int c, int fd) {
    char b = c;
    int len;
...
static void z_flushbuf() {
    int len = outptr - outbuf;
    if (len != 0) {
        if (lastfd != -1) z_write(lastfd, outbuf, len);
        outptr = outbuf;
    }
}
...
void z_vprintf(const char *fmt, va_list ap) { kdoprnt(2, fmt, ap); }
...
void z_vfdprintf(int fd, const char *fmt, va_list ap) { kdoprnt(fd, fmt, ap); }

static void kdoprnt(int fd, const char *fmt, va_list ap) {
    unsigned long ul;
    int lflag, ch;
    char *p;
    static int init;
...
static void kprintn(int fd, unsigned long ul, int base) {
    /* hold a long in base 8 */
