...
int *z_perrno(void) { return &errno; }
...
#define DEF_SYSCALL1(ret, name, t1, a1) \
    ret z_##name(t1 a1) { return (ret)SYSCALL(name, a1); }
#define DEF_SYSCALL2(ret, name, t1, a1, t2, a2) \
    ret z_##name(t1 a1, t2 a2) { return (ret)SYSCALL(name, a1, a2); }
#define DEF_SYSCALL3(ret, name, t1, a1, t2, a2, t3, a3) \
    ret z_##name(t1 a1, t2 a2, t3 a3) { return (ret)SYSCALL(name, a1, a2, a3); }
...
void *z_mmap(void *addr, size_t length, int prot, int flags, int fd,
             off_t offset) {
#if defined(__i386__)
