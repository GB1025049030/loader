...
static unsigned long loadelf_anon(int fd, Elf_Ehdr *ehdr, Elf_Phdr *phdr) {
    ...
    for (iter = phdr; iter < &phdr[ehdr->e_phnum]; iter++) {
        ...
        if (z_lseek(fd, iter->p_offset, SEEK_SET) < 0) goto err;
...
void exec_elf(const char *file, int argc, char *argv[]) {
    ...
    for (i = 0;; i++, ehdr++) {
        ...
        if (!check_ehdr(ehdr)) z_errx(1, "bogus ELF header %s", file);
        ...
        if (file == elf_interp) break;
        for (iter = phdr; iter < &phdr[ehdr->e_phnum]; iter++) {
            if (iter->p_type != PT_INTERP) continue;
